...
private def split_cmdline_args : list string → io (string × list string × list string)
| [] := io.fail "split_cmdline_args: no args provided"
| [cmd] := return (cmd, [], [])
...
def main : io unit :=
do (cmd, outer_args, inner_args) ← io.cmdline_args >>= split_cmdline_args,
