...
static environment set_simp_sizeof(environment const & env, name const & n) {
...
class mk_has_sizeof_fn {
    ...
    expr mk_add(expr const & A, expr const & B) const { return mk_app(m_add, A, B); }
    ...
    void define_instance() {
        ...
        if (is_inductive_predicate(m_env, m_ind_name))
            return;
        ...
        buffer<expr> indices;
        {
            ...
            for (unsigned param_idx = 0; param_idx < num_params; ++param_idx) {
                expr param = m_tctx.push_local_from_binding(ty);
            ...
            for (expr const & param : params) {
                if (auto inst_arg_type = build_has_sizeof_argument_type(param)) {
                    expr param_inst = m_tctx.push_local(local_pp_name(param).append_after("_inst"), *inst_arg_type, mk_inst_implicit_binder_info());
...
public:
    mk_has_sizeof_fn(environment const & env, name const & ind_name):
        ...
        m_one(mk_app(mk_constant(get_one_name(), {mk_level_one()}), {mk_constant(get_nat_name()), mk_constant(get_nat_has_one_name())})),
        m_add(mk_app(mk_constant(get_add_name(), {mk_level_one()}), {mk_constant(get_nat_name()), mk_constant(get_nat_has_add_name())}))
        {}
