...
class add_basic_inductive_decl_fn {
    ...
    void send_to_kernel() {
        ...
        lean_trace(name({"inductive_compiler", "basic", "ind"}), tout() << c_ind << "\n";);
        ...
        for (expr const & ir : intro_rules) {
            ...
            expr new_ir_type = infer_implicit_params(Pi(params, replace_local(mlocal_type(ir), ind, c_ind)), params.size(), k);

            // DEBUG
            if (has_local(new_ir_type)) {
                collected_locals cls;
                collect_locals(new_ir_type, cls);
                buffer<expr> locals = cls.get_collected();
                lean_trace(name({"inductive_compiler", "basic", "irs"}), tout() << locals.size() << "\n";);
                unsigned i = 0;
            }
            // END
