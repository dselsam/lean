...
unsigned lport_exporter::export_name(name const & n) {
...
unsigned lport_exporter::export_level(level const & l) {
...
void lport_exporter::export_binder_info(binder_info const & bi) {
    if (bi.is_implicit())
        m_out << "#BI";
...
unsigned lport_exporter::export_const(expr const & e) {
    buffer<unsigned> ls;
...
lport_exporter::lport_exporter(std::ostream & out) : m_out(out) {
    m_name2idx[{}] = 0;
    m_level2idx[{}] = 0;
}
