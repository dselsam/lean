...
static Eigen::ArrayXXf read_mnist_images(string full_path, int& number_of_images, int& image_size) {
    ...
    if(file.is_open()) {
        ...
        Eigen::ArrayXXf mat(number_of_images, image_size_images);
...
vm_obj eigen_read_mnist(vm_obj const & shape, vm_obj const & _filename, vm_obj const &, vm_obj const &) {
    std::string filename = to_string(_filename);
    std::ifstream in(filename);
