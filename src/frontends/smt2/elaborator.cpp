...
struct arith_app_info {
    ...
    name const & get_int_inst_name() const { return m_int_inst; }
    name const & get_real_inst_name() const { return m_real_inst; }
...
private:
    ...
    expr elaborate_app(buffer<expr> & args) {
        ...
        if (it_special != g_special_map->end()) {
            ...
            case special_app_kind::EQ: return elaborate_eq(args);
            case special_app_kind::ITE: return elaborate_ite(args);
            case special_app_kind::DISTINCT: return elaborate_distinct(args);
