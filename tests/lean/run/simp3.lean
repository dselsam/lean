...
/-
example : @distrib.to_has_mul.{1} A (@ring.to_distrib.{1} A (@comm_ring.to_ring.{1} A A_cr)) = (@comm_semigroup._trans_of_to_semigroup.{1} A (@comm_ring.to_comm_semigroup.{1} A A_cr)) :=
by dsimp >> trace "state:" >> trace_state >> reflexivity
--by simp >> trace "state:" >> trace_state >> triv
-/
