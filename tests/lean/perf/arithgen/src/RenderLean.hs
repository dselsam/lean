...
leanHeader numVars = "import algebra.ring\n"
                     ...
                     ++ "meta_definition arith : tactic unit := do (new_target, Heq) ← target >>= arith_normalize, assert \"Htarget\" new_target, triv, Ht ← get_local \"Htarget\", mk_app (\"eq\" <.> \"mpr\") [Heq, Ht] >>= exact\n"
                     ++ "end tactic\n"
                     ++ "open tactic\n"
                     ++ "open tactic\n"
