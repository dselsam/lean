...
| mk : list A -> wrap A -> box

-- inductive foo.{l} : Type.{max 1 l}
-- | mk : box foo -> foo

mutual_inductive foo, fbox, fwrap, flist
with foo : Type
| mk : fbox -> foo
with fbox : Type
| mk : flist -> fwrap -> fbox
...
attribute [reducible] definition fbox₂.mk (xs : list foo) (x : fwrap) : fbox₂ := fbox.mk (pack_flist xs) x
attribute [reducible] definition fwrap₂.mk (x : foo) : fwrap₂ := fwrap.mk x
...
definition fbox₂.rec (C : fbox₂ → Type)
                     (mp : Pi (a : list foo) (x : fwrap), C (fbox₂.mk a x))
                     (x : fbox₂) : C x :=
@fbox.rec C
          (λ (a : flist) (x : fwrap),
            @eq.rec_on flist
                       (pack_flist (unpack_flist a))
                       (λ (a : flist), C (fbox.mk a x))
                       a
                       (flist_pack_unpack a)
                       (mp (unpack_flist a) x))
          x
