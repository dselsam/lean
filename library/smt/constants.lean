...
-- Skip ITE instances
-- Note: We may want to move more (or less) functionality into this method
| (app (app (app (app (app (const `ite [level.one]) cond) _) ty) e₁) e₂) :=
    list.union (list.union (list.union (collectConstants cond) (collectConstants ty)) (collectConstants e₁)) (collectConstants e₂)
