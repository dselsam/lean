...
-- BitVec
| _ (app (app (const `zero _) (app (const `BitVec []) e)) _) :=
       do (sz, _) ← toMaybeNat e,
          szMacro ← return $ mkNumeralMacro sz ConcreteArithType.nat,
          some $ app (const `BitVec.zero []) szMacro
| _ (app (app (const `one _) (app (const `BitVec []) e)) _)  :=
       do (sz, _) ← toMaybeNat e,
          szMacro ← return $ mkNumeralMacro sz ConcreteArithType.nat,
          some $ app (const `BitVec.one []) szMacro

| _ (app (app (app (app (const `add _) (app (const `BitVec []) e)) _) e₁) e₂) :=
       do (sz, _) ← toMaybeNat e,
          szMacro ← return $ mkNumeralMacro sz ConcreteArithType.nat,
          t₁ ← strip tt e₁, t₂ ← strip tt e₂, some $ app (app (app (const `BitVec.add []) szMacro) t₁) t₂
| _ (app (app (app (app (const `mul _) (app (const `BitVec []) e)) _) e₁) e₂) :=
       do (sz, _) ← toMaybeNat e,
          szMacro ← return $ mkNumeralMacro sz ConcreteArithType.nat,
          t₁ ← strip tt e₁, t₂ ← strip tt e₂, some $ app (app (app (const `BitVec.mul []) szMacro) t₁) t₂
...
| b e := match toMaybeNat e with
         ...
         | some (n, const `nat [])  := some $ mkNumeralMacro n ConcreteArithType.nat
...
meta def stripTheoryTypeclasses : tactic unit :=
do tgt ← target,
