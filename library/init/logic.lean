...
xinductive Exists {A : Type} (P : A → Prop) : Prop
...
xinductive [class] decidable (p : Prop) : Type
| ff : ¬p → decidable
| tt :  p → decidable
...
xinductive [class] inhabited (A : Type) : Type
| mk : A → inhabited
...
xinductive [class] nonempty (A : Type) : Prop
| intro : A → nonempty
...
xinductive [class] subsingleton (A : Type) : Prop
