...
theorem not_congr_eq [congr] (H : a = b) : (¬a) = (¬b)
 :=
sorry
...
theorem forall_congr_eq [congr] [priority 0] {A : Type} {P Q : A → Prop} (H : ∀a, (P a = Q a)) : (∀a, P a) = ∀a, Q a :=
sorry

theorem imp_congr_eq [congr] [priority std.priority.default+1] (H1 : a = c) (H2 : b = d) : (a → b) = (c → d) :=
sorry
...
theorem exists_congr_eq [congr] {A : Type} {P Q : A → Prop} (H : ∀a, (P a = Q a)) : (∃a, P a) = ∃a, Q a :=
sorry
...
namespace contextual
  attribute if_ctx_simp_congr
  attribute if_ctx_simp_congr_prop
  attribute dif_ctx_simp_congr
end contextual
