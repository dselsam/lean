...
theorem hfunext_full {A A' : Type} {B : A → Type} {B' : A' → Type} {f : Π a : A, B a} {g : Π a' : A', B' a'}
                     (HA : A = A') (H : ∀ (a:A) (a':A'), a == a' → f a == g a') : f == g :=
by induction HA ; apply hfunext ; exact (λ a, H a a (heq.refl a))
